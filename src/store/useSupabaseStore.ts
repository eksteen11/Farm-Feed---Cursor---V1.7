  ...
  persist(
    (set, get) => ({
      ...
      register: async (userData: Partial<User>, password: string) => {
        set({ isLoading: true, error: null })
        try {
          // Format userData for Supabase metadata (only include simple types)
          const metadata = {
            name: userData.name,
          ...
          const result = await SupabaseAuthService.signUp(userData.email!, password, metadata)
          
          if (result.error) {
            console.error('❌ Registration error:', result.error)
            const errorMessage = typeof result.error === 'string' 
              ? result.error 
              : result.error?.message || JSON.stringify(result.error) || 'Registration failed'
          ...
          // If not, try to create it manually as fallback
          try {
            const profileCheck = await SupabaseAuthService.getCurrentUser()
            if (!profileCheck) {
              console.log('⚠️ Profile not created by trigger, attempting manual creation...')
              // Try to manually create profile if trigger failed
              // This is a fallback - the trigger should handle it, but this ensures it works
            }
          } catch (profileError) {
            console.warn('Profile check failed (this is OK if trigger hasn\'t run yet):', profileError)
          }

          // Success - auth user created
          // If email confirmation is disabled, user can sign in immediately
          // If enabled, they'll need to check email (but signup succeeded)
          set({
            isLoading: false,
            error: null,
          })
          ...
          set({
            error: error?.message || error?.toString() || 'Registration failed. Please try again.',
            isLoading: false,
          })
          return false
        }
