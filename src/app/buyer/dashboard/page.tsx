...
export default function BuyerDashboardPage() {
  ...
  const { offers, transportRequests, listings } = useStore()
  ...
  const renderOffers = () => {
    // Filter listings to only include those with sellers (required by Listing type)
    const validListings = listings.filter((listing): listing is Listing => listing.seller !== undefined)
    return (
      <BuyerOffers
        offers={buyerOffers}
        listings={validListings.length > 0 ? validListings : (mockListings as unknown as Listing[])}
