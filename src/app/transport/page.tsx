...
export default function TransportPage() {
  const { currentUser, getCurrentUser, transportRequests: storeTransportRequests, transportQuotes: storeTransportQuotes } = useSupabaseStore()
  ...
  const [transportRequests, setTransportRequests] = useState<TransportRequest[]>(storeTransportRequests.length > 0 ? storeTransportRequests : mockTransportRequests)
  ...
  // Update transport requests when store updates
  useEffect(() => {
    if (storeTransportRequests.length > 0) {
      setTransportRequests(storeTransportRequests)
    }
  }, [storeTransportRequests])
  ...
  const handleQuoteSubmit = (e: React.FormEvent) => {
    ...
    if (!canUserPerformAction(currentUser, 'transport')) {
      toast.error('You need transport capability to submit quotes')
      return
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Action Items - UX Workflow: What Needs User Attention */}
        {isAuthenticated && currentUser && (
          <div className="mb-12">
            <h2 className="text-headline text-gray-900 mb-8">Action Items</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Pending Quotes to Review (For Requesters) */}
              {transportRequests.filter(r => 
                r.requesterId === currentUser.id && 
                r.status === 'quoted'
              ).length > 0 && (
                <Card variant="elevated" className="border-l-4 border-[#DB4A39]">
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-lg font-semibold text-gray-900">
                        Pending Quotes to Review
                      </h3>
                      <span className="px-3 py-1 bg-[#DB4A39]/10 text-[#DB4A39] rounded-full text-sm font-semibold">
                        {transportRequests.filter(r => r.requesterId === currentUser.id && r.status === 'quoted').length}
                      </span>
                    </div>
                    <p className="text-gray-600 mb-4">
                      You have {transportRequests.filter(r => r.requesterId === currentUser.id && r.status === 'quoted').length} transport request(s) with quotes waiting for your review
                    </p>
                    <Button 
                      onClick={() => router.push('/transport/my-requests')}
                      className="w-full"
               ...
               transportRequests.filter(r => r.status === 'open').length > 0 && (
                <Card variant="elevated" className="border-l-4 border-green-500">
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-lg font-semibold text-gray-900">
                        Requests Needing Quotes
                      </h3>
                      <span className="px-3 py-1 bg-green-100 text-green-800 rounded-full text-sm font-semibold">
                        {transportRequests.filter(r => r.status === 'open').length}
                      </span>
                    </div>
                    <p className="text-gray-600 mb-4">
                      {transportRequests.filter(r => r.status === 'open').length} transport request(s) are open and need your quotes
                    </p>
                    <Button 
                      onClick={() => router.push('/transport/available')}
                      className="w-full"
              ...
              {/* Jobs In Transit (For Transporters) */}
              {canUserPerformAction(currentUser, 'transport') && 
               transportRequests.filter(r => r.status === 'in_progress' && r.transporterId === currentUser.id).length > 0 && (
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <span className="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm font-semibold">
                        {transportRequests.filter(r => r.status === 'in_progress' && r.transporterId === currentUser.id).length}
                      </span>
                    </div>
                    <p className="text-gray-600 mb-4">
                      Update status for {transportRequests.filter(r => r.status === 'in_progress' && r.transporterId === currentUser.id).length} active transport job(s)
                    </p>
                    <Button 
                      onClick={() => router.push('/transport/my-requests')}
                      className="w-full"
              ...
              {/* No Action Items */}
              {transportRequests.filter(r => r.requesterId === currentUser.id && r.status === 'quoted').length === 0 &&
               (!canUserPerformAction(currentUser, 'transport') || transportRequests.filter(r => r.status === 'open').length === 0) &&
               (!canUserPerformAction(currentUser, 'transport') || transportRequests.filter(r => r.status === 'in_progress' && r.transporterId === currentUser.id).length === 0) && (
                <Card variant="elevated" className="border-l-4 border-green-500">
                  <CardContent className="p-6">
        ...
        {/* Transport Dashboard - Use Real Data */}
        <ClientOnly>
          <TransportDashboard 
            userId={currentUser?.id} 
            userRole={currentUser?.role}
            requests={transportRequests}
            quotes={storeTransportQuotes.length > 0 ? storeTransportQuotes : []}
          />
        </ClientOnly>
          ...
          {filteredRequests.map((request) => (
            <Card key={request.id} className="hover:shadow-lg transition-shadow">
              <CardContent className="p-6">
                ...
                {/* Requester with Trust Signals */}
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-2">
                    <img
                      src={(request.requester as any)?.avatar || "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=150&h=150&fit=crop&crop=face"}
                      alt={request.requester.name}
                      className="w-6 h-6 rounded-full"
                    />
                    <div className="flex items-center space-x-2">
                    <span className="text-sm font-medium">{request.requester.name}</span>
                      {(request.requester as any)?.isVerified && (
                  ...
                  <div className="flex items-center space-x-1 text-sm text-gray-500">
                    ...
                    <span>{(request.requester as any)?.rating || '4.8'}</span>
                    {(request.requester as any)?.transactionCount && (
                ...
                <div className="flex space-x-2">
                  ...
                  {request.status === 'open' && canUserPerformAction(currentUser, 'transport') && (
                    <Button 
                      className="flex-1" 
                      size="sm"
                      onClick={() => router.push(`/transport/${request.id}`)}
              ...
              <div className="bg-gray-50 rounded-lg p-4 mb-6">
                ...
                <div className="mt-3 pt-3 border-t border-gray-200">
                  <p className="text-xs text-gray-500">
                    <span className="font-medium">Platform Fee:</span> R300 total (R150 from buyer, R150 from seller) - This fee is separate from your quote price
                  </p>
                </div>
              </div>
