...
import { User, Offer, Listing } from '@/types'
...
export default function OffersPage() {
  const { currentUser, offers, listings, users, fetchOffers, fetchListings } = useSupabaseStore()
  ...
  madeOffersRaw.forEach(offer => {
    ...
    unifiedOffers.push({
      ...
      requiresAction: offer.status === 'pending' || offer.status === 'counter-offered'
    })
  })
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'counter-offered': return <AlertCircle className="w-4 h-4" />
