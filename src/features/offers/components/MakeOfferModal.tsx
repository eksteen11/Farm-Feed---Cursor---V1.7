...
export default function MakeOfferModal({ 
  ...
  const [formData, setFormData] = useState({
    ...
    quantity: Math.min(10, listing.availableQuantity || listing.quantity || 1),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!listing?.sellerId && !listing?.seller?.id) {
      toast.error('Listing seller information is missing')
      return
    }
    ...
    try {
      ...
      const offerData = {
        ...
        seller_id: listing.sellerId || listing.seller?.id,
