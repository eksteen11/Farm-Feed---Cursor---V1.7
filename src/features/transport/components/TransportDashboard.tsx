...
export default function TransportDashboard({ userId, userRole, requests = [], quotes = [] }: TransportDashboardProps) {
  ...
  useEffect(() => {
    ...
    const activeRequests = requests.filter(r => ['open', 'quoted', 'accepted', 'in_progress'].includes(r.status)).length;
    const completedRequests = requests.filter(r => r.status === 'completed' || r.status === 'delivered').length;
    const pendingQuotes = quotes.filter(q => q.status === 'pending').length;
    const totalEarnings = quotes
      .filter(q => q.status === 'accepted')
      .reduce((sum, q) => sum + q.price, 0);
    const acceptedQuotes = quotes.filter(q => q.status === 'accepted');
    const averageDeliveryTime = acceptedQuotes.length > 0
      ? acceptedQuotes.reduce((sum, q) => sum + (q.estimatedDays || 0), 0) / acceptedQuotes.length
      : 0;
