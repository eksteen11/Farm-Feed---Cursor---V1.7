...
export default function SimpleMaps({ height = '600px', className = '' }: SimpleMapsProps) {
              ...
              >
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-gray-600 mb-4">Interactive map coming soon with Google Maps integration</p>
                    
                    {/* Location Dots */}
                    <div className="relative w-80 h-60 mx-auto">
                      {/* Free State */}
                      <div className="absolute top-16 left-24 w-3 h-3 bg-green-500 rounded-full animate-pulse cursor-pointer" 
                           onClick={() => setSelectedLocation(DEMO_LOCATIONS[0])}
                           title="Free State Farm">
                      </div>
                      {/* Gauteng */}
                      <div className="absolute top-20 left-28 w-3 h-3 bg-green-500 rounded-full animate-pulse cursor-pointer"
                           onClick={() => setSelectedLocation(DEMO_LOCATIONS[1])}
                           title="Gauteng Farm">
                      </div>
                      {/* Western Cape */}
                      <div className="absolute top-32 left-8 w-3 h-3 bg-green-500 rounded-full animate-pulse cursor-pointer"
                           onClick={() => setSelectedLocation(DEMO_LOCATIONS[2])}
                           title="Western Cape Farm">
                      </div>
                      {/* KZN */}
                      <div className="absolute top-28 left-32 w-3 h-3 bg-red-500 rounded-full animate-pulse cursor-pointer"
                           onClick={() => setSelectedLocation(DEMO_LOCATIONS[3])}
                           title="Transport Route">
                      </div>
                      {/* Mpumalanga */}
                      <div className="absolute top-24 left-30 w-3 h-3 bg-blue-500 rounded-full animate-pulse cursor-pointer"
                           onClick={() => setSelectedLocation(DEMO_LOCATIONS[4])}
                           title="Backload Opportunity">
                      </div>
                      {/* Northern Cape */}
                      <div className="absolute top-18 left-20 w-3 h-3 bg-green-500 rounded-full animate-pulse cursor-pointer"
                           onClick={() => setSelectedLocation(DEMO_LOCATIONS[5])}
                           title="Northern Cape Farm">
                      </div>
                    </div>
